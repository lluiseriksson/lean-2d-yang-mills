...
shared by two faces.  Combined with the all-label coefficient extraction
already proved in the same development, this yields the exact normalized
simple-loop identity
\[
  \mathbb E[W_n(C)]
...
\begin{lstlisting}
theorem su2_exact_simpleLoop_areaLaw
    {t : Real} (ht : 0 < t) (n : Nat) :
  (integral (fun g : SU2 =>
    su2NormalizedWilsonCharacter n g *
      ...
      su2HaarProb) =
    ((Real.exp (-t * ((n : Real) *
      ((n : Real) + 2) / 4)) : Real) : Complex)
\end{lstlisting}

Together with the Migdal move, this is the exact two-dimensional route: edge
subdivision does not change the density, and the remaining simple face has the
Casimir coefficient prescribed by its area.
...
in the Lean development.  Printing axioms for the twelve new public endpoints
...
The present artifact closes the compact-group analytic chain needed for an
exact simple-loop calculation.  It does not yet formalize arbitrary embedded
planar graphs, gauge fixing on a general cellular decomposition, nonsimple or
intersecting loops, continuum measure construction, or higher-genus partition
functions.  Those are independent extensions and should not be inferred from
the two-face theorem.

The next mathematically meaningful consumer is a finite planar-cellulation
reduction theorem: repeated application of the concrete Migdal move, with
incidence and orientation data made explicit, until a simple loop bounds one
effective face.  That step would promote the local subdivision identity into a
global discretization-independence statement.
